...
export function add_canvas(sizeP) {
  var canvas = document.getElementById('game_canvas');
  if (!canvas) return;
  var ctx = canvas.getContext("2d");

  const rem = sizeP.remValue;
  const cW = sizeP.gamePartW; 
  const cH = sizeP.gamePartH; 
  ...
  ctx.beginPath();
  ctx.lineWidth = 0.3 * rem; // РђРґР°РїС‚РёРІРЅС‹Р№ Р°РЅР°Р»РѕРі С‚РІРѕРµР№ С‚РѕР»С‰РёРЅС‹ 3
  ctx.strokeStyle = "#6B4C21";

  for (var i = 0; i < 4; i++) {
    // Р РёСЃСѓРµРј Р»РѕС‚РѕРє РїРѕ С‚РІРѕРёРј РѕСЂРёРіРёРЅР°Р»СЊРЅС‹Рј РєРѕСЌС„С„РёС†РёРµРЅС‚Р°Рј РІС‹СЃРѕС‚С‹ Рё С€РёСЂРёРЅС‹
    ctx.moveTo(cW * posX[i][0], cH * (0.27 + posY[i]));
    ctx.lineTo(cW * posX[i][1], cH * (0.27 + posY[i]));
    ctx.lineTo(cW * posX[i][2], cH * (0.385 + posY[i]));

    // РџРѕРґРїРѕСЂРєР° Р»РѕС‚РєР°
    ctx.moveTo(cW * posX[i][3], cH * (0.35 + posY[i]));
    ctx.lineTo(cW * posX[i][4], cH * (0.45 + posY[i]));
    ctx.stroke();
  }
 ...
 ctx.beginPath();
  ...
  // ==========================================
  var setRad = [20, 8, 8, 20, 8]; 
  var flagBush = 1;
  var setX = [1, 2.95, 16.7, 18.8, 18.8], setY = [1, 1.04, 1.04, 1, 0.1]; 
  
  ctx.beginPath();
  ctx.lineWidth = 0.1 * rem; 
  for (var i = 0; i < 5; i++) {
    var currentRad = (setRad[i] / 10) * rem; 
    
    ctx.moveTo(cW * 0.05 * setX[i] + currentRad, cH * 0.7 * setY[i]);
    ctx.arc(cW * 0.05 * setX[i], cH * 0.7 * setY[i], currentRad, 0, Math.PI * 2, true);
    
    for (var j = 0; j < 12; j++) {
      var degX = cW * 0.05 * setX[i] - currentRad * Math.sin(360 / 12 / 180 * Math.PI * (j + 1));
      var degY = cH * 0.7 * setY[i] - currentRad * Math.cos(360 / 12 / 180 * Math.PI * (j + 1));
      ctx.moveTo(degX, degY);
      
      var leafSize = ((5 + flagBush) / 10) * rem;
