...
export class PromiseGame{
 ...
 createTimerPromiseRabbit(obj : HTMLElement, obj_next : HTMLElement, val : string, time : number, result: number) {
 ...
 createTimerPromise(obj : HTMLElement, obj_next : HTMLElement, time : number, result: number) {
 ...
 createTimerPromise2(obj_next : HTMLElement, result: number) {
  return new Promise<number>( (resolve,reject) => {
     setTimeout( () => {
       if(!obj_next)
         reject("игра окончена!!!");       
